...
- **Root Control Page**: `/` serves a shadcn-inspired browser control panel that talks to the same protobuf API, with stable per-card loading skeletons, compact/full card text controls, title-row fullscreen card viewing, and touch-friendly card reordering with edge auto-scroll.
