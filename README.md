...
- **Root Control Page**: `/` serves a shadcn-inspired Yew/WebAssembly browser control panel with direct Radix icons that talks to the same protobuf API, with readable shadcn dark destructive controls, compact mobile stats, stable per-card loading skeletons, compact/full card text controls, searchable archive filters, a remembered grid/column flow layout switch, readable-width list expansion, syntax-highlighted markdown code blocks, solid Unified Flow cards when more than eight cards are visible, title-row fullscreen card viewing that hides background cards, old-template-compatible tipcard actions, and touch-friendly handle-based card reordering with edge auto-scroll.
