...
- `review_and_advance` applies SM-2 scheduling and returns the next occupant of
  the repeatable topic slot (promoted pending card, or an already-due sibling)
  in one replayable transaction. Its result directly contains the replacement
  `FlowCardInfo`, or completion/refill state when no card is ready.
