...
pub async fn pick_topic_icon(
    ...
    let response = create_chat_completion(model, &prompt, api_key, api_base, reasoning).await;
...
mod tests {
    ...
    fn parse_topic_icon_response_reads_json_object() {
        assert_eq!(
            super::parse_topic_icon_response("{\"icon\":\"lucide:code\"}"),
    ...
    fn parse_topic_icon_response_reads_fenced_json() {
        assert_eq!(
            super::parse_topic_icon_response("```json\n{\"icon\":\"tabler:server\"}\n```"),
