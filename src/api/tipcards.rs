...
pub(crate) async fn delete_tipcard_by_id(state: &AppState, id: i64) -> ApiResult<()> {
    tipcards::delete_with_review(&state.db, id)
        .await
        .map_err(|err| err.into_status_body())
}

pub async fn set_tipcard_pinned(state: &AppState, id: i64, pinned: bool) -> ApiResult<()> {
...
pub(crate) async fn active_card_room(
    ...
    let active = tipcards::active_card_count(&state.db)
        .await
        .map_err(|err| err.into_status_body())?
        .max(0) as u64;
