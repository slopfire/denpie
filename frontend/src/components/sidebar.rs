        ...
        <nav class="hidden lg:flex fixed left-0 top-0 z-50 h-full w-56 flex-col border-r surface p-4 rounded-none">
            ...
            <div class="space-y-1 flex-1">
                <Link<View> to={View::Flow} classes={classes!("nav-item", "w-full", "grid", "grid-cols-[1.5rem_minmax(0,1fr)]", "items-center", "gap-3", "rounded-md", "px-3", "py-2", "text-sm", "font-semibold", "text-left", (props.current_view == View::Flow).then_some("active"))}>
                    <iconify-icon icon="tabler:antenna" class="radix-icon justify-self-center"></iconify-icon><span class="justify-self-start text-left">{i18n.t("nav.flow")}</span>
                </Link<View>>
                <Link<View> to={View::Grounding} classes={classes!("nav-item", "w-full", "grid", "grid-cols-[1.5rem_minmax(0,1fr)]", "items-center", "gap-3", "rounded-md", "px-3", "py-2", "text-sm", "font-semibold", "text-left", (props.current_view == View::Grounding).then_some("active"))}>
                    <iconify-icon icon="tabler:circuit-ground" class="radix-icon justify-self-center"></iconify-icon><span class="justify-self-start text-left">{i18n.t("nav.grounding")}</span>
                </Link<View>>
